...
import { getStudentWrongQuestions } from '@/lib/teaching/practice-agent';
...
export async function GET() {
  ...
  if (user.role !== 'student' || !user.studentId) {
    return apiError('INVALID_REQUEST', 403, '仅学生账号可查看错题集');
  }
  try {
    const wrongQuestions = await getStudentWrongQuestions(
      DEFAULT_TEACHING_COURSE_ID,
      user.studentId,
    );
    return apiSuccess({ wrongQuestions });
  } catch (err) {
    return apiError(
      'INTERNAL_ERROR',
      500,
      err instanceof Error ? err.message : 'Failed to load wrong questions',
    );
  }
}
