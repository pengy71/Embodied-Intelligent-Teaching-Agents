...
import { AlertTriangle, BarChart3, Download, RefreshCw, Users, TrendingUp, Target, ClipboardList } from "lucide-react";
...
export default function TeacherOverviewPage() {
  ...
  const completionData = analytics?.completionDistribution.length ? analytics.completionDistribution : DEFAULT_COMPLETION;
  const questionTrend = analytics?.questionTrend.length ? analytics.questionTrend : DEFAULT_WEEKS;
  const testDistribution = analytics?.testDistribution.length ? analytics.testDistribution : DEFAULT_TEST;
  const errorDistribution = analytics?.errorDistribution.length ? analytics.errorDistribution : DEFAULT_ERRORS;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="课程概览"
        description="实时掌握班级整体学习情况、知识掌握程度及薄弱知识点"
      >
        <Button variant="outline" size="sm" onClick={() => loadAnalytics(true)} disabled={isLoadingAnalytics}>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        {[
          { label: "班级人数", value: summary.totalStudents, desc: `今日活跃 ${summary.activeToday}`, icon: Users, color: "text-primary" },
          { label: "平均进度", value: `${summary.averageProgress}%`, desc: "学习完成度", icon: TrendingUp, color: "text-emerald-600" },
          { label: "平均掌握率", value: `${summary.averageMastery}%`, desc: "知识点掌握", icon: Target, color: "text-amber-600" },
          { label: "预警学生", value: summary.warningCount, desc: "需要关注", icon: AlertTriangle, color: "text-rose-600" },
          { label: "练习记录", value: activity.practiceCount, desc: `问答 ${activity.qaCount}`, icon: ClipboardList, color: "text-violet-600" },
          { label: "学习事件", value: activity.totalEvents, desc: `复习 ${activity.reviewCount}`, icon: BarChart3, color: "text-sky-600" },
        ].map((item) => {
          ...
          <CardContent className="space-y-2">
            {hotQuestions.length > 0 ? (
              hotQuestions.map((question, index) => (
                <div key={`${question.topic}-${index}`} className="flex items-center gap-3 rounded-lg border p-3">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary/10 text-sm font-semibold text-primary">
                    {index + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium">{question.topic}</p>
                    <p className="truncate text-xs text-muted-foreground">{question.knowledgePoint}</p>
                  </div>
                  <span className="shrink-0 text-xs text-muted-foreground">{question.count} 次</span>
                </div>
              ))
            ) : (
              ...
              suggestions.map((item) => (
                <div key={item.title} className="rounded-lg border p-3">
                  <div className="mb-1 flex items-center gap-2">
                    <Badge variant={item.tag === "重点讲解" ? "destructive" : item.tag === "课堂讨论" ? "secondary" : "default"}>
                      {item.tag}
                    </Badge>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      完成度 {student.completionRate}% | 掌握率 {student.mastery}% | 测验 {student.testScore} 分
                    </p>
                  </div>
