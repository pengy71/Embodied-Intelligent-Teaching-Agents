...
console.log("DATABASE_URL  ->", new URL(url).host + new URL(url).pathname, "(port " + new URL(url).port + ")");
console.log("DIRECT_URL    ->", new URL(direct).host + new URL(direct).pathname, "(port " + new URL(direct).port + ")");
...
try {
  ...
  console.log("  已装扩展:", ext.rows.map((r) => `${r.extname}@${r.extversion}`).join(", ") || "(无 vector/pgcrypto)");

  const tables = await pool.query(
    "select table_name from information_schema.tables where table_schema='public' order by table_name",
  );
  console.log("  public 表:", tables.rows.length ? tables.rows.map((r) => r.table_name).join(", ") : "(空)");

  const rowCnt = await pool.query(
